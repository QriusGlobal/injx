...
def test_get_with_async_provider_raises():
    """Test that get() raises for async providers in sync context."""
    container = Container()
    ...
    # get() should raise (sync context with async provider)
    with pytest.raises(ResolutionError) as exc_info:
        _ = container.get(token)
